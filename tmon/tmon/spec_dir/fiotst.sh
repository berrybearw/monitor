tdate=`date +%Y%m%d`
logdir=/u1/etc/tmon/spec_dir/fio/log/
logfile=${logdir}fio.log
mkdir -p $logdir
cd /u1/etc/tmon/spec_dir/fio-3.16
start_time=`date`
...
#隨機讀
cd $logdir
fio -direct=1 -iodepth=128 -rw=randread -ioengine=libaio -bs=4k -size=1G -numjobs=1 -runtime=10 -group_reporting -filename=iotest_randr${tdate} -name=Rand_R_Test > fio_randr.log
rm -rf iotest_randr${tdate}
fio_randr=`grep 'IOPS=' ${logdir}fio_randr.log`

echo "FIO 隨機寫入測試中"
#隨機寫
fio -direct=1 -iodepth=128 -rw=randwrite -ioengine=libaio -bs=4k -size=1G -numjobs=1 -runtime=10 -group_reporting -filename=iotest_randw${tdate} -name=Rand_W_Test > fio_randw.log
...
echo "FIO 混合隨機讀寫測試中"
#混合隨機讀寫
fio -direct=1 -iodepth=128 -rw=randrw -rwmixread=70 -ioengine=libaio -bs=4k -size=1G -numjobs=1 -runtime=10 -group_reporting -filename=iotest_randrw${tdate} -name=Rand_RW_Test > fio_randrw.log
...
cat $logfile
echo ""
echo "以上為IOPS數值，若有需查看詳細測試結果請至/u3/bin/fio/log目錄內查看log"
